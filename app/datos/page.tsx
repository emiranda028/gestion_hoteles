...
export default async function Page() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="titulo">Datos e ingesta automática</h1>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <section className="tarjeta rounded-xl border border-neutral-200 bg-white p-4 shadow-sm">
          <h2 className="mb-2 text-sm font-semibold text-neutral-700">Última ejecución</h2>
          {d.demo && (
          ...
          {d.ingesta ? (
            ...
            <p className="text-sm text-neutral-500">La ingesta todavía no se ejecutó.</p>
          )}
        </section>

        <section className="tarjeta rounded-xl border border-neutral-200 bg-white p-4 shadow-sm">
          <h2 className="mb-2 text-sm font-semibold text-neutral-700">Descargas</h2>
          {d.demo ? (
            <p className="text-sm text-neutral-500">Disponibles cuando haya datos reales.</p>
          ) : (
            <ul className="space-y-1 text-sm">
              ...
              <li><a className="text-marca underline" href="/api/descargar/paises.csv">paises.csv</a> — huéspedes por país (Marriott BA)</li>
            </ul>
          )}
      ...
      <section className="tarjeta rounded-xl border border-neutral-200 bg-white p-4 shadow-sm">
        <h2 className="mb-2 text-sm font-semibold text-neutral-700">Cobertura por hotel (últimos 30 días)</h2>
            ...
            <tbody>
              {cobertura.map((c) => (
                <tr key={c.id} className="border-t border-neutral-100 align-top">
                  ...
                  <td className={`py-2 pr-4 ${c.hasta < d.hasta ? 'text-acento' : ''}`}>{fechaLarga(c.hasta)}</td>
